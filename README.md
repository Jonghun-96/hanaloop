...
```bash
# 1. 저장소 클론
git clone https://github.com/[YOUR_ID]/pcf-dashboard.git && cd pcf-dashboard

# 2. 환경변수 설정
cp .env.example .env
# .env의 DATABASE_URL 수정 (또는 아래 Docker 사용 시 그대로 둬도 됨)
...
**.env 기본값 (Docker 사용 시)**
```
DATABASE_URL="postgresql://pcf_user:pcf_pass@localhost:5432/pcf_dashboard"
```
...
| 검토 | 직접 | 계산 공식 검증, 단위 확인 | — |

**직접 결정한 사항**: GHG Scope 분류 기준, 배출계수 버전 관리 방식, 단일 PCF API 설계
...
**시간이 많이 소요된 부분**: Recharts 커스텀 스타일링과 다크 테마 CSS 조정에 예상보다 시간이 걸렸습니다. Recharts의 Tooltip/Legend가 전역 CSS를 따르지 않아 개별 prop으로 스타일을 지정해야 했습니다.
